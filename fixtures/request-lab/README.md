...
ReqFreedom 的本地请求功能验证页：既提供可被规则命中的静态资源，也提供一组动态接口，用来观测规则改写后**服务端实际收到了什么**。
...
- 规则匹配上却无法应用的两种情形：不透明响应（no-cors）与同步 XHR
...
另有两条规则专门验证「匹配上但未能应用」：`不透明响应上的 Mock` 与 `同步 XHR 上的 Mock`。它们对应同名卡片，预期都不产生命中，而是在 popup 中以黄色禁止图标标出原因——这两种情形都是浏览器限制导致的 fail-open，不是规则本身有问题。
...
| `/api/cross-origin/opaque` | 供 `mode: 'no-cors'` 请求取得不透明响应：请求成功但响应体不可读 |
